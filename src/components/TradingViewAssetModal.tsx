...
export const TradingViewAssetModal: React.FC<TradingViewAssetModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'chart' | 'marketwatch' | 'technicals' | 'profile' | 'news' | 'holdings'>('chart');
  ...
  const [useProxySymbol, setUseProxySymbol] = useState<boolean>(Boolean(tvDetails.embedRestricted && tvDetails.proxyTvSymbol));

  // Determine active symbol to render in the widget
  const activeTvSymbol = (useProxySymbol && tvDetails.proxyTvSymbol) ? tvDetails.proxyTvSymbol : tvDetails.tvSymbol;
      ...
      <div className="bg-white dark:bg-[#0c1017] border-0 rounded-none w-full h-full max-w-full max-h-full shadow-2xl overflow-hidden flex flex-col text-slate-800 dark:text-slate-200 transition-colors">
          ...
          <div className="flex items-center gap-2 self-end lg:self-center">
            ...
            {tvDetails.marketWatchUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-md shadow-emerald-600/20 cursor-pointer"
                title={`Open MarketWatch for ${tvDetails.ticker} (Path: ${tvDetails.marketWatchPath})`}
              >
        ...
        <div className="px-4 sm:px-6 bg-slate-100 dark:bg-[#0a0e15] border-b border-slate-200 dark:border-white/10 flex items-center gap-1 sm:gap-2 overflow-x-auto no-scrollbar shrink-0">
          {[
            ...
            ...(tvDetails.marketWatchUrl ? [{ id: 'marketwatch', label: 'MarketWatch (PSE Live Feed)', icon: TrendingUp }] : []),
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-4 bg-slate-50/70 dark:bg-[#080b10]">
          ...
          {/* TAB: MARKETWATCH PSE LIVE FEED & TOTAL VALUATION */}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    ...
                    <span>MarketWatch (PH) Spot Quotation & Valuation Feed</span>
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    Official MarketWatch ticker integration for Philippine Stock Exchange (PSE) equities and REIT investments.
                  </p>
                </div>
                {tvDetails.marketWatchUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 self-start sm:self-auto shadow-sm transition-all"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                    <span>Open MarketWatch.com</span>
                  </a>
                )}
              </div>

              {/* MarketWatch Real-Time Quote Card */}
              <div className="p-5 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 rounded-2xl shadow-xs space-y-5">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-white/5">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-black text-xl">
                      🇵🇭
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-base font-black text-slate-900 dark:text-white">{tvDetails.name}</span>
                        <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 rounded font-mono font-black text-xs">
                          {tvDetails.ticker} (PH)
                        </span>
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 rounded font-bold text-[10px]">
                          PSE Equities
                        </span>
                      </div>
                      <div className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">
                        MarketWatch Path: <code className="text-indigo-600 dark:text-indigo-400 font-bold">{tvDetails.marketWatchPath || `/investing/stock/${tvDetails.ticker.toLowerCase()}?countrycode=ph`}</code>
                      </div>
                    </div>
                  </div>

                  <div className="flex items-end flex-col">
                    <span className="text-xs text-slate-400 font-semibold">Current Price Quotation</span>
                    <div className="text-2xl font-black font-mono text-slate-900 dark:text-white">
                      ₱{currentPricePHP.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                    </div>
                    <span className={`inline-flex items-center gap-1 text-xs font-mono font-bold px-2 py-0.5 rounded mt-1 ${
                      isPositive
                        ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300'
                        : 'bg-rose-100 text-rose-800 dark:bg-rose-500/20 dark:text-rose-300'
                    }`}>
                      {isPositive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                      <span>{isPositive ? '+' : ''}{changePct.toFixed(2)}% (24h Trend)</span>
                    </span>
                  </div>
                </div>

                {/* 4 Core Valuation Metric Blocks */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">Units Held</span>
                    <span className="text-lg font-black font-mono text-slate-900 dark:text-white">
                      {asset.units.toLocaleString()} shares
                    </span>
                    <span className="text-[10px] text-slate-400 block font-mono">
                      Cost: ₱{asset.costBasisPHP.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>

                  <div className="p-4 bg-emerald-50/50 dark:bg-emerald-500/5 rounded-xl border border-emerald-200 dark:border-emerald-500/20 space-y-1">
                    <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase block">Total Valuation (PHP)</span>
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block font-mono">
                      Solved as Units × Spot Price
                    </span>
                  </div>

                  <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">Unrealized Net P&L</span>
                    <span className={`text-lg font-black font-mono ${unrealizedGainPHP >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      {unrealizedGainPHP >= 0 ? '+' : ''}₱{unrealizedGainPHP.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                    <span className={`text-[10px] font-bold font-mono ${unrealizedGainPct >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      {unrealizedGainPct >= 0 ? '+' : ''}{unrealizedGainPct.toFixed(2)}% ROI
                    </span>
                  </div>

                  <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">Market Source</span>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block">
                      MarketWatch / PSE Feed
                    </span>
                ...
                {/* Direct Link Banner */}
                <div className="p-3.5 bg-slate-100 dark:bg-white/[0.02] rounded-xl border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    <Globe className="w-4 h-4 text-slate-400 shrink-0" />
                    <span className="text-slate-600 dark:text-slate-400">
                      MarketWatch URL: <a href={tvDetails.marketWatchUrl} target="_blank" rel="noopener noreferrer" className="text-emerald-600 dark:text-emerald-400 font-mono font-bold hover:underline">{tvDetails.marketWatchUrl}</a>
                    </span>
                  </div>
                  {tvDetails.marketWatchUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold flex items-center gap-1 transition-all shrink-0 cursor-pointer shadow-2xs"
                    >
                      <ExternalLink className="w-3 h-3" />
                      <span>Launch MarketWatch</span>
                    </a>
                  )}
                </div>
              </div>
